(function(designWidth, rem2px) {
    var win = window; // 拿到window对象
    var doc = win.document; // 拿到doc对象
    var docEl = doc.documentElement; // 拿到html
    var metaEl = doc.querySelector('meta[name="viewport"]'); // 查询viewport
    var dpr = 0;
    var scale = 0;
    var tid;

    if (!dpr && !scale) {
      var devicePixelRatio = win.devicePixelRatio; // 拿到dpr
      if (win.navigator.appVersion.match(/iphone/gi)) { //非iphone 不执行高清
    ...
    docEl.setAttribute('data-dpr', dpr); // 在html根节点上添加一个data-dpr属性 为目前设备的dpr值
    if (!metaEl) {  //viewport 不存在 
